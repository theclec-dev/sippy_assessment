...
class _ProductsListPageState extends State<ProductsListPage> {
  ...
  void didChangeDependencies() {
    ...
    final items = Provider.of<ProductsList>(context).products;
    ...
    for (final item in items) {
      ...
      _isLoading.putIfAbsent(item.id.toString(), () => false);
    }
    // Dispose & remove controllers for items no longer present
    final removedKeys = _controllers.keys
        .where(
          (id) => items.every((item) => item.id.toString() != id),
        )
        .toList();
    for (final id in removedKeys) {
      _controllers.remove(id)?.dispose();
      _isLoading.remove(id);
    }
  }
                    ...
                    delegate: SliverChildBuilderDelegate(
                      (context, index) {
                        ...
                        return ProductCard(
                          inCart: inCart,
                          controller:
                              _controllers[data.products[index].id.toString()]!,
                          product: data.products[index],
                          isLoading:
                              _isLoading[data.products[index].id.toString()] ??
                                  false,
                          onTapPrimary: () async {
                            final sharedCartServices = SharedCartServices();
                            setState(() {
                            ...
                            try {
                              ...
                              await sharedCartServices.addItem(
                                ...
                                quantity: int.parse(_controllers[
                                        data.products[index].id.toString()]!
                                    .text),
                                userId: userId,
                                userName: userName,
